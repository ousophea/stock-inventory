<?php 
	...
	echo form_open('problem/solve/'.$stock->stock_id,$attr);
		...
		echo '<table>';
			...
			echo '<tr>';
				echo '<td>Assign By</td><td>:</td><td>';						
				echo $stock->username.'</td></tr>';
			
			echo '<tr valign="top">';
				echo '<td>Message</td><td>:</td><td>';					 
					$val = (isset($_POST['msg'])?$_POST['msg']:$stock->msg);
					$data = array('name'=>'msg','value'=>$val);
				echo form_textarea($data).'</td>';
			echo '</tr>';
			...
			echo '<tr>';
				...
				echo '<td>
					<div class="buttons">
					    ...
					    echo anchor('problem',img(base_url().'assets/images/icons/cross.png').'Cancel','class="negative"');				
						echo form_hidden('id',$stock->pro_id);
						echo form_hidden('assign_by',$stock->assign_by);			
				    echo '</div>';	
				echo '</td>';
			echo '</tr>';
