...
?>
			...
			<div class="form_search">
				<form name="frmsearch" action="" method="post">				
					...
					<?php
						...
						$url_search = site_url('stock/search');
					?>
	...
	<form name="frm" action="" method="post">
			...
			<?php
				echo anchor('stock/add',img($path.'assets/images/icons/add.png').'Add New');				
			?>
			<a href="javascript:void(0);" onclick="doSubmit(document.frm,'delete','stock/deleteall');" title="Delete">
		...
		<div class="tab" style="padding:5px 5px 5px 0;">
			<?php				
				echo anchor('stock','Stock <span>('.$instock->stock.')</span>','id="stock"').' | ';
				echo anchor('stock/used','Used <span>('.$used->used.')</span>','id="used"').' | ';
				echo anchor('stock/problem','Problem <span>('.$problem->problem.')</span>','id="problem"').' | ';
				$total = ($instock->stock+$used->used+$problem->problem);
				...
				<td>
					<?php
						if($row['pro_id']){
							...
							echo anchor('stock/assign/'.$row['stock_id'],'Assign');
						}
						echo ' | ';
						echo anchor('stock/delete/'.$row['stock_id'],'Delete');
					?>
				</td>			
