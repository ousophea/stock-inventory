...
class Stock extends Base_Controller {
	...
	{		
		...
		$config['total_rows']=$this->obj->stock_num(1);
		$pageNumber = 15;
		...
		$data['listStock'] = $this->obj->stock(1,$pageNumber,$this->uri->segment(3));
	...
	{		
		...
		//todo add stylesheet with pagging
		$config['anchor_class'] = 'class="pagging"';
		$config['total_rows']=$this->obj->stock_num(2);
		$pageNumber = 15;
		$config['per_page']=$pageNumber;
		$config['num_links']=5;
		$config['uri_segment'] = 3;
		...
		$data['listStock'] = $this->obj->stock(2,$pageNumber,$this->uri->segment(3));
		$this->load->view('stock/stock_message',$data);
	}
	...
	{		
		...
		//todo add stylesheet with pagging
		$config['anchor_class'] = 'class="pagging"';
		$config['total_rows']=$this->obj->stock_num(3);
		$pageNumber = 15;
		$config['per_page']=$pageNumber;
		$config['num_links']=5;
		$config['uri_segment'] = 3;
		...
		$data['listStock'] = $this->obj->stock(3,$pageNumber,$this->uri->segment(3));
		$this->load->view('stock/stock_problem_message',$data);
	}
	...
	function assign($id=0){
		$data['stock'] = $this->obj->get_data_by('view_stock','stock_id',$id);
		$data['users'] = $this->combox_user();
		...
		if($this->form_validation->run() == FALSE){			
			$this->load->view('stock/assign_stock_problem',$data);
		}else{		
			$this->add_assign();
			redirect($this->page.'/problem','');			
		}
	}
	function combox_user(){
		$result = array();
		$result['0'] = '-- Please select --';
		$data = $this->obj->get_all_data_by('tbl_user','active',1,'user_id');
		foreach($data as $row){
	...
	function add_assign(){
		$date = date('Y-m-d H:m:s');
		$id = $this->input->post('id');
		$solve_by = $this->input->post('ddUser');
		$data = array(
					'stock_id' => $id,
		...
		$tmp = $this->obj->save("tbl_problem",$data);
		
		if($tmp){
			$theme_view = 'public/email_assigned';
			
			$from = $this->obj->get_email_by_user($this->userId);
			$from_name = $this->obj->get_user_name($this->userId);
			
			$email = $this->obj->get_email_by_user($solve_by);
			$to_name = $this->obj->get_user_name($solve_by);
			
			$tmpdata = $this->obj->get_data_by('view_stock','stock_id',$id);
