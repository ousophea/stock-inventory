...
class Item extends Base_Controller {
	...
	{		
		//set parameter of pagination	
		$data['listItem'] = $this->obj->get_all_data_by('tbl_history','stock_id',$id,'stock_id');
	...
	function save(){
		...
		$loc = $this->obj->get_data_by('tbl_location','loc_id',$location);
		...
		$data_h = array(
							...
							'status' => 'Used',
							'username' => get_cookie('inventory_user_name'),
							'date_import' => $row->date_import,
							'date_created' => date('Y-m-d H:i:s'),
							'remark' => $remark,
							'do_action' => 'saved'
						);
		$this->obj->save('tbl_history',$data_h);
	...
	function update(){
		...
		$status_name = ($status==1?'Stock':($status==2?'Used':'Problem'));
